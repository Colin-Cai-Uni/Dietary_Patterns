...
def main():
    ...
    if args.cluster_column is None:
        data['cluster'] = 0
    ...
    populations = data[args.cluster_column].value_counts(sort = False).values
...
def barplot(column, cluster, data, labels, directory, destination):
    counts = []
    for l in labels:
        count = data[data[cluster] == l][column].value_counts().rename(l)
    ...
    plot_counts = pd.melt(counts, id_vars = 'value', var_name = 'cluster', value_name = 'proportion')

    fig = sns.catplot(x = 'value', y = 'proportion', hue = 'cluster', data = plot_counts, 
                      kind = 'bar', aspect = 0.5*len(values))
    ...
    for p in ax.patches:
        ax.text(p.get_x() + p.get_width()/2.0, p.get_height(), '%.2f' % round(p.get_height(), 2), 
